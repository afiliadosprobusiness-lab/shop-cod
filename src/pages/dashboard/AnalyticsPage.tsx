...
import { BarChart3, DollarSign, Lock, MousePointerClick, Sparkles, Users } from "lucide-react";
...
import { getAdvancedAnalyticsAccess } from "@/lib/plans";
import {
...
export default function AnalyticsPage() {
  ...
  return (
    <MainContent
      ...
      actions={
        !access.allowed ? (
          <Button type="button" className="rounded-2xl" onClick={() => setUpgradeModalOpen(true)}>
            Desbloquear analiticas avanzadas
          </Button>
        ) : null
      }
    >
      ...
      {access.allowed ? (
        ...
        <section className="rounded-[2rem] border border-primary/20 bg-card/90 p-6">
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_18rem]">
            <div className="space-y-4">
              ...
              <h2 className="text-2xl font-semibold text-foreground">
                Las analiticas avanzadas requieren el plan Pro
              </h2>
              <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                Tu plan actual muestra los KPIs basicos, pero las curvas de ventas, productos mas
                vendidos, page views y checkouts iniciados se desbloquean desde Pro.
              </p>
              <div className="grid gap-3 sm:grid-cols-3">
                <div className="rounded-2xl border border-border/80 bg-secondary/20 p-4 text-sm text-muted-foreground">
                ...
                <div className="rounded-2xl border border-border/80 bg-secondary/20 p-4 text-sm text-muted-foreground">
                  Trafico y checkouts iniciados
                </div>
              </div>
            </div>

            <div className="rounded-[1.75rem] border border-border/80 bg-secondary/20 p-5">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-primary/10 text-primary">
                <Sparkles className="h-5 w-5" />
              </div>
              <p className="mt-4 text-lg font-semibold text-foreground">Desbloquea Pro</p>
              <p className="mt-2 text-sm text-muted-foreground">
                Activa analiticas avanzadas y sigue optimizando tu conversion.
              </p>
              <Button
                type="button"
      ...
      <PlanUpgradeDialog
        ...
        featureName="Analiticas avanzadas"
        reason={access.reason}
        requiredPlanId={access.requiredPlan.id}
      />
    </MainContent>
  );
}
